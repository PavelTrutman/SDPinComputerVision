\chapter{Semidefinite programming}\labelcha{SDP}
...
We will focus in details on SDP application in polynomial optimization and polynomial systems solving in \refcha{POP}.
...
At the beginning, let us denote the inner product for two vectors $x$, $y \in\R^n$ by
\begin{align}
...
See \reffig{SDP:prelim:problem} to get a general picture, how a simple semidefinite problem may look like.
...
SDPA \cite{sdpa} uses an infeasible interior-point method, which can be initialized by an infeasible point.
...
\section{Interior point method}\labelsec{SDP:nesterov}
...
\begin{definition}[Self-concordant function in $\R^n$]
  A closed convex function $f\colon~\R^n \mapsto \R$ is self-concordant if function $g\colon \R \mapsto \R$
  \begin{align}
    g(t) &= f(x + tv)
  \end{align}
...
\begin{theorem}[{\thecite[Theorem~4.1.1]{Nesterov-2004}}]
...
\begin{corollary}[{\thecite[Corollary~4.1.2]{Nesterov-2004}}]\labelcol{SDP:scf:scaling}
...
\begin{theorem}[{\thecite[Theorem~4.1.3]{Nesterov-2004}}]\labelthe{SDP:scf:hessian}
...
For some self-concordant function $f(x)$, for which we assume that $\dom f$ contains no straight line (which implies that all $f''(x)$ are nondegenerate, see \refthe{SDP:scf:hessian}), we introduce two local norms as
\begin{align}
...
\refalg{SDP:scf} describes an iterative process of solving the optimization problem \refeqb{SDP:scf:minProblem}.
...
\begin{definition}[Self-concordant barrier {\thecite[Definition~4.2.2]{Nesterov-2004}}]\labeldef{SDP:scb:scb}
...
\begin{theorem}[{\thecite[Theorem~4.2.1]{Nesterov-2004}}]\labelthe{SDP:scb:penaltyFunction}
  Let $F(x)$ be a self-concordant barrier. Then, the function $c^\top x + F(x)$ is a self-concordant function on $\dom F$.
\end{theorem}

\begin{theorem}[{\thecite[Theorem~4.2.2]{Nesterov-2004}}]
...
\begin{theorem}[{\thecite[Theorem~4.2.5]{Nesterov-2004}}]\labelthe{SDP:scb:distance}
...
\begin{definition}[{\thecite[Definition~4.2.3]{Nesterov-2004}}]
...
\begin{theorem}[{\thecite[Theorem~4.2.6]{Nesterov-2004}}]
...
\begin{theorem}[{\thecite[Theorem~4.2.10]{Nesterov-2004}}]
...
\begin{theorem}[{\thecite[Theorem~4.2.8]{Nesterov-2004}}]
...
\begin{theorem}[{\thecite[Theorem~4.2.9]{Nesterov-2004}}]
...
\refalg{SDP:scb:ac} and \refalg{SDP:scb:pf} can be easily used to solve the standard minimization problem \refeqb{SDP:scb:standardProblem}, supposing we have a feasible point $x_0\in Q$.
...
\begin{theorem}[{\thecite[Theorem~4.3.3]{Nesterov-2004}}]\labelthe{SDP:bf:nu}
...
\begin{example}
  ...
  This situation is shown in \reffig{SDP:bf:barrier} for different values of $t$.
...
When the matrix $X$ has the block diagonal form \refeqb{SDP:bf:bd}, we can rewrite the barrier function \refeqb{SDP:bf:bfy} as
\begin{align}
...
\begin{example}\labelex{SDP:imp:demo}
  ...
  The graph of the problem is shown in \reffig{SDP:imp:demo}.
...
Our intention was to measure only the execution time of the solving phase, not of the setup time.
...
For overall comparison we have also measured the setup time, e.g.\ the execution time spent before the SDP solver is actually called, of all three packages and we have shown them in \reffig{SDP:perf:times}.
...
\subsection{Results}
By the look of the solving times shown in \reffig{SDP:perf:times}, we can see that the MOSEK toolbox totally wins.
...
However, due to SeDuMi overhead, the Polyopt package is faster than SeDuMi for problem sizes up to eight.

Regarding the setup times of the solvers, we can observe that the YALMIP framework spends more execution time by analyzing and compiling the problem than by solving it.
This makes inadequate overhead for small problems, but it may prove crucial for solving large problems with many unknowns.

\section{Speed--accuracy trade-off}\labelsec{SDP:sa}
Obtaining a precise solution of a semidefinite program is quite time consuming, especially for problems with many unknowns, as we can see from \reffig{SDP:perf:times}.
...
The first experiment is an observation how many iterations, and therefore how much time, is required to find a solution based on the value of $\varepsilon$ from \refalg{SDP:scb:pf}.
This value of $\varepsilon$ represents an accuracy of \refalg{SDP:scb:pf} and sets up a threshold for the termination condition.
...
To evaluate the dependency we have generated unique semidefinite programs $P_{k, l}$ according to \refeqb{SDP:perf:problem} for $k = \importSDPPrecEpsDims{}$ and $l = 1,\ldots,\importSDPPrecEpsUnique{}$.
For each of this problem and accuracy $\varepsilon$ we have measured the number of iterations $N_{k,l,\varepsilon}$ of \refalg{SDP:scb:pf} required to find the solution.
...
\end{align}
The values of $N_{k,\varepsilon}$ are plotted in \reffig{SDP:perf:eps} for different values of $k$ and $\varepsilon$.

\begin{figure}[ht]
  \centering
  \resizebox{0.95\textwidth}{!}{\input{graphs/SDP_prec_eps_iters}}
  \caption{Graph of numbers of iterations required to solve the semidefinite problems by \refalg{SDP:scb:pf} for different values of problem size $k$ based on $\varepsilon$ using the implementation from the Polyopt package.}
  \labelfig{SDP:perf:eps}
\end{figure}
...
This results is in accordance with the estimated number of steps from the equation \refeqb{SDP:scb:pfSteps}.

\subsection{Analysis based on the required distance from the solution}
\input{macros/SDP_prec_perc}
\refalg{SDP:scb:pf} starts at the analytic center $y_F^*$ of the feasible set of the problem and then follows the central path until it is sufficiently close to the optimal solution $y^*$.
...
The situation for a simple semidefinite problem is shown in \reffig{SDP:perf:perc:demo}.

\begin{figure}[ht]
  \centering
  \resizebox{0.95\textwidth}{!}{\input{graphs/SDP_prec_perc_demo}}
  \caption{Example of a simple semidefinite problem with steps of \refalg{SDP:scb:pf}. The algorithm starts from the analytic center $y^*_{F}$ and finishes at the optimal point $y^*$. Selected fractions of the distance $\|y^* - y_F^*\|$ are represented by concentric circles.}
  \labelfig{SDP:perf:perc:demo}
\end{figure}

To see how many iterations are needed to get within some fraction of the distance $\|y^* - y_F^*\|$ from the optimal solution $y^*$, we have generated unique semidefinite problems $P_{k, l}$ for the problem sizes $k = \importSDPPrecPercDims{}$ and $l = 1, \ldots, \importSDPPrecPercUnique{}$ according to \refeqb{SDP:perf:problem}.
...
\end{align}
The values of $N_{k,\lambda}$ are plotted in \reffig{SDP:perf:perc} for different values of $k$ and $\lambda$.

\begin{figure}[ht]
  \centering
  \resizebox{0.95\textwidth}{!}{\input{graphs/SDP_prec_perc_iters}}
  \caption{Graph of numbers of iterations required to get within the distance $\lambda \|y^* - y_F^*\|$ from the optimal solution $y^*$ using \refalg{SDP:scb:pf} for different values of problem size $k$ based on $\lambda$ using the implementation from the Polyopt package.}
  \labelfig{SDP:perf:perc}
\end{figure}

From the experiments we can see that the number of iterations, and therefore the execution time, of \refalg{SDP:scb:pf} required to get within some fraction $\lambda$ of the distance $\|y^* - y_F^*\|$ for a given problem with fixed problem size $k$ is almost proportional to $\log(\lambda)$.
