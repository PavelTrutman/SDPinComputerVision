...
We have solved these problems by our implementation from the Polyopt package and by selected the state of the art toolboxes, namely SeDuMi \cite{sedumi} and MOSEK \cite{mosek}.
...
Since in typical applications of SDP, the size of the matrices grows with the number of variables, we have set $m = n$ to have just single parameter, which we call size of the problem.
...
\begin{eqnarray}
  X_{k,l}(y) &=& \Iden k + \sum_{i=1}^k A_{k,l,i} y\ind i
\end{eqnarray}
for $k = \importSDPPerformanceDimMin, \ldots, \importSDPPerformanceDimMax$ and $l = 1,\ldots, \importSDPPerformanceUnique$, where $A_{k,l,i}\in\Sym^k$.
The matrices $A_{k,l,i}$ are filled by random numbers from uniform distribution $(-1;1)$ with symmetricity of the matrices preserved.
...
Therefore, we add constraint \refeqb{SDP:imp:bound} for $R = \importSDPPerformanceBound$, which will guarantee that we will optimize over bounded set.
...
The final generated problem denoted $P_{k,l}$ looks like
\begin{eqnarray}
  \begin{array}{rclrrclcll}
...
To eliminate influences that negatively affect the execution times on CPU, such as other processes competing for the same CPU core, processor caching, data loading delays, we have executed each problem $P_{k,l}$ \importSDPPerformanceRepeat{} times.
So, for each problem $P_{k,l}$ we have obtained execution times $\tau_{k,l,s}$ for $s = 1,\ldots,\importSDPPerformanceRepeat$.
Because the influences mentioned above can only prolong the execution times, we have selected minimum of $\tau_{k,l,s}$ for each $P_{k,l}$
\begin{eqnarray}
  \tau_{k,l} &=& \min_{s=1}^{\importSDPPerformanceRepeat} \tau_{k,l,s}.
\end{eqnarray}
Since the execution times of problems of the same sizes should be more or less the same, we have computed the average execution time $\tau_k$ for each problem size.
\begin{eqnarray}
  \tau_k &=& \frac{1}{\importSDPPerformanceUnique}\sum_{l=1}^{\importSDPPerformanceUnique} \tau_{k,l}
\end{eqnarray}
These execution times $\tau_k$ were measured and computed separately for the Polyopt, SeDuMi and MOSEK toolboxes and are shown in \reftab{SDP:perf:times} and \reffig{SDP:perf:times}, where $k$ is the size of the problem.
...
Fortunately, YALMIP measures the execution time of the solver, so we have used this time in our statistics.
