...
%TODO: cite lapack, atlas, blas
The incorrect setting of these libraries causes significant drop of the performance.
Other Python packages are required as well, e.g.\ SymPy\cite{sympy} and SciPy\cite{scipy}, but theirs settings are not so crucial for the performance of this implementation.
%TODO: cite sympy, scipy
...
To install other packages that are required, the preferred way is to use the pip\cite{pip} installer. The required packages are listed in the \texttt{requirements.txt} file.
%TODO: cite pip
...
\end{lstlisting}
To check, whether the installation was successful, simply run command \texttt{python3 setup.py test} and the predefined tests will be automatically run.
...
\begin{eqnarray}
  \begin{array}{rclrrclcll}
    y^* &=& \displaystyle \inf_{y\in\R^m} & \multicolumn{3}{l}{c^\top y} \\
