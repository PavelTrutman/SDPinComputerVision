...
plot[Min:Max][0:] "data/app_P35Pf_times.dat" using (offset($1,1)):(1.0) title "Automatic generator \\cite{autogen}" smooth freq with boxes fill solid noborder lc 2,\
                  "data/app_P35Pf_times.dat" using (offset($2,2)):(1.0) title "Polyopt" smooth freq with boxes fill solid noborder lc 3,\
                  "data/app_P35Pf_times.dat" using (offset($3,3)):(1.0) title "MATLAB implementation with MOSEK \\cite{mosek}" smooth freq with boxes fill solid noborder lc 4,\
                  "data/app_P35Pf_times.dat" using (offset($4,4)):(1.0) title "Gloptipoly \\cite{gloptipoly}" smooth freq with boxes fill solid noborder lc 7
