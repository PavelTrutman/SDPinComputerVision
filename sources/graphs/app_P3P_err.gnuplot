...
offset(x, y) = bin(x)+(y-ceil(datafiles/2.0))*boxwidth

set boxwidth boxwidth absolute

plot[Min:Max][:] "data/app_P3P_err.dat" using (offset($1,1)):(1.0) title "Ground truth" smooth freq with boxes fill solid noborder ,\
                 ...
                 "data/app_P3P_err.dat" using (offset($4,4)):(1.0) title "MATLAB implementation with MOSEK" smooth freq with boxes fill solid noborder ,\
